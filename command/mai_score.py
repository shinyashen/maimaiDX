...
from ..libraries.maimai_best_50 import generate, generate40, generateTags
...
b50_tags= sv.on_prefix(['b50标签', 'b50tags'])
...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    for i in ev.message:
        if i.type == 'at' and i.data['qq'] != 'all':
            qqid = int(i.data['qq'])
    await bot.send(ev, await generateTags(qqid, username), at_sender=True)
